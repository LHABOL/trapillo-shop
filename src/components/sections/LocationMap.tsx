      ...
      <div className="container-editorial grid gap-12 md:grid-cols-2 md:items-center">
        ...
        >
          <svg viewBox="0 0 400 300" className="h-full w-full" preserveAspectRatio="xMidYMid slice" aria-label="Mapa estilizado de la ubicación">
            <rect width="400" height="300" fill="#E2D5BF" />
            {Array.from({ length: 9 }).map((_, i) => (
              <line key={`h${i}`} x1="0" x2="400" y1={i * 38 + 10} y2={i * 38 + 4} stroke="#CDB89A" strokeWidth="6" />
            ))}
            {Array.from({ length: 12 }).map((_, i) => (
              <line key={`v${i}`} x1={i * 36 + 8} x2={i * 36 + 2} y1="0" y2="300" stroke="#CDB89A" strokeWidth="5" />
            ))}
            <path d="M-10 210 C 120 190, 260 250, 420 200" stroke="#A9835B" strokeWidth="14" fill="none" opacity="0.6" />
            <path d="M150 -10 C 170 120, 120 200, 200 320" stroke="#A9835B" strokeWidth="12" fill="none" opacity="0.5" />
            <circle cx="205" cy="150" r="30" fill="#6F4E37" opacity="0.14" />
            <circle cx="205" cy="150" r="7" fill="#3E2C20" />
            <path d="M205 150 L205 120" stroke="#3E2C20" strokeWidth="3" />
          </svg>
